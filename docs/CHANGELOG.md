# 修订记录（CHANGELOG）

> 对应手册 3.3：文档末尾保留按日期的修订记录，写清每次逻辑定型的原因（哪些方案被实测推翻）。
> 新条目写在最上面。

---

## 2026-09-15 · v2.0.11 两项反馈（写稿模板加子任务保留百分比 / 长按拖动排序任务）

来源：腾讯文档《专注小站迭代log》v2.0.11 页，两条「未修复」反馈。

- **写稿模板底部加子任务，时间百分比保留**（反馈②）：
  - 原逻辑：`AddStage` 插入新子任务后调用 `RenumberStages()`，把所有子任务百分比**整体重排成均匀线性**（100/90/80/70/60…），会覆盖写稿模板（100/90/80/65/20）等手工调好的递减序列。
  - 改为：插入时**不重排已有行**；新子任务按「继续递减」取值——底部追加取 `末行-10`（下限 1），中间插入取相邻两行百分比的中点（仅当相邻差 ≥2 时能放下合法值，否则回退到整体重排以保证合法性）。各子任务「剩余 %」与对应时长均保留。
  - 说明：腾讯文档原句「时间百分比需要改成下图这样的」配图无法以文本获取，按「继续原有递减序列、不破坏已调好的百分比」这一最贴合意图的方式实现；如配图另有具体格式，请对照后告知。
  - 回归：`UiTests.cs` 新增断言——载入写稿模板后加子任务，原有 100/90/80/65/20 不变，新行取值合法且严格递减。
- **长按 2 秒拖动任务改变排序**（反馈③）：
  - 主面板任务列表（`projects` 自定义 `TaskListBox`）原无拖动排序；仅有「复制 / 删除」右键。
  - 新增：在任务上**按住左键 2 秒**进入排序模式（光标变 `SizeNS`、状态栏提示「拖动排序中…松手保存顺序」），移动即实时重排显示，松手把新顺序写入 `data.Projects` 并保存；单击 / 短按仍正常选中，移动超过 8px 视为点击取消长按。
  - 回归：`UiTests.cs` 新增断言——`ReorderVisible`+`CommitReorder` 把可见任务顺序落到 `data.Projects`，隐藏（已删除 / 另一完成态）任务位置不变。
- **验证**：`scripts/build.ps1` 通过；`scripts/test.ps1`（逻辑单测 + 界面单测）全绿。

## 2026-09-14 · 提前完成的步骤不再在原定时间点弹窗（TIMER-004）

用户反馈：挑战模式下提前勾掉前几步，到这些步骤的原定时间点仍会弹出「已完成当前路径点」并暂停计时，提醒下一步。

- **根因**：挑战的阶段提醒由 `FocusEngine.Tick` 里的 `stageIndex` 按「逻辑时间越过计划边界」推进；而提前完成（子任务清单勾选、mini 勾选）只把 `Stage.Done=true`，`stageIndex` 不动。时间越过原定边界时，引擎仍把早已完成的步骤当作「刚到达」，触发 stage 弹窗并暂停。
- **改动**：
  - `src/Model.cs`：`Tick` 推进 `stageIndex` 时跳过 `Done` 的阶段——提前完成的步骤不再触发 stage 事件；后续未完成步骤仍在各自原定边界提醒，总挑战时长不变（与「提前完成」弹窗承诺的「后续原定节点不变」一致）。
  - `src/MainForm.cs`：番茄钟模式下，子任务清单勾选/取消勾选后调用 `RefreshStageDeadline()`（对齐「完成当前一步」「撤回上一步」路径），提醒跟随当前未完成步骤的截止点，修掉番茄钟下的同类旧时间点弹窗。
- **回归**：`Tests.cs` 新增 6 条断言（提前完成的步骤在老边界保持专注不弹窗；只提前一段时，下一段仍在自己的边界弹窗且 `Reached` 只含未完成段；提前完成不改变总挑战时限）；`UiTests.cs` 新增番茄钟勾选后提醒重新对准下一步的断言。登记 `docs/事故与回归映射.md` TIMER-004。
- **验证**：`powershell scripts/test.ps1` 通过（逻辑 168 断言 + 界面单测）；`build.ps1` dev 产物构建通过。

## 2026-09-08 · v2.0.10 用户反馈三项（申请加时默认值 / 复制任务 / 迷你条可见性）

来源：腾讯文档《专注小站迭代log》v2.0.10 页三条反馈。

- **申请加时默认值**（反馈①）：
  - 原逻辑：默认加时 =「剩余时长」的 1/3，且单次上限被 120 分钟硬顶（`Math.Min(120, projectLimit)`，弹窗内还有一处隐藏的 120 上限）。
  - 改为：默认 = **该子任务分配时长的 1/3**（`TaskSupport.StageSeconds/180`），用户仍可拖动/输入调节；单次上限 = **项目初始总时长**（`ChallengeBaseMinutes`），去掉了 120 分钟硬顶（`ChallengeNoteDialog` 内的 `Math.Min(maxMinutes,120)` 一并移除）。
- **复制任务**（反馈②，按反馈给出的第二方案实现，未引入模板存储）：
  - 任务列表右键新增「复制任务（名称、描述与子任务）」：只复制任务名、描述、完成标准和子任务清单（标题/动作/时长分配），**完成状态清零**；计时不记录、历史不复制。
  - 新任务名自动编号：复制「写作练习」→「写作练习（1）」；复制「写作练习（1）」→「写作练习（2）」，与现存任务名去重。等价于轻量「存为模板再新建」；若后续需要真正的模板库再单独立项。
- **迷你计时条有时看不到**（反馈③）：
  - 根因：`MiniTimer` 的屏幕定位只在第一次 `Shown` 事件里做一次；换显示器 / 改分辨率后窗口可能留在屏幕外，且无兜底。
  - 改为：每次 `ShowMini()` 显示后立即把窗口位置钳制回当前屏幕工作区，恢复最小化状态并 `Activate()` 置前。
- **验证**：`build.ps1` 通过；`test.ps1` 162 断言 + 界面单测（含 mini 各状态截图）通过。

## 2026-09-08 · 事故映射与可观测性文档同步

- 新增 `docs/事故与回归映射.md`，为数据、计时、任务、同步、AI、UI、日志事故建立编号和回归入口。
- 确认并记录 `RuntimeLogDialog` 已提供本地运行日志查看、范围筛选、CSV 导出和清空；日志不上传、不参与 AI 分析。
- 更新项目方案与工程实践对照表，移除“结构化事件日志缺口”和“事故闭环缺口”的过时描述。

## 2026-09-08 · 运行日志（把只写不读的事件记录接完）

- **问题**：dev 上的 `a151d2d` 混进了一套 `RuntimeEvent` 事件记录，`FocusEngine` 在开始/暂停/继续/结束/无响应时写入 `data.xml`，但没有任何界面读取它 —— 纯负担：文件只增不减，用户看不到，出了问题也无从查起。
- **改动**：
  - `src/Model.cs`：补齐埋点（新增 完成阶段 / 申请加时 / 进入休息 / 完成任务）；新增 `RuntimeLog` 类，提供事件中文名、专注段计算、今日小结、CSV 导出、清空。
  - `src/Dialogs.cs`：新增 `RuntimeLogDialog` —— 顶部今日小结、范围筛选（今天 / 最近 7 天 / 全部）、事件表格、导出 CSV、清空。
  - `src/MainForm.cs`：设置页「数据文件夹」旁新增「运行日志」按钮。
  - `src/Tests.cs`：新增 `RuntimeLogChecks`（23 条断言），覆盖埋点、段落时长、小结统计、CSV、上限裁剪。
- **设计取舍**：
  - 段落时长取自计时记录求和，而不是事件时间相减 —— 保证与「历史」页的数字一致，且自动排除暂停时间。
  - 未知事件名原样显示而非显示「其他」—— 以后新增埋点不会退化成无意义标签。
  - 清空日志需二次确认，并明确说明不影响项目与计时记录；上限仍为 2000 条，超出丢弃最旧的。
- **验证**：`test.ps1` 通过（160 断言 + UI 单测）；dev 构建产物内含「运行日志」「今天启动了」。
- **用途**：回答「为什么老启动不了」—— 今天启动了几次、累计多久、最长一段多长、断了多少次，一眼可见。

---

## 2026-09-08 · dev 数据隔离（数据目录与同步配置分离）

- **问题**：`src/Program.cs` 把数据目录写死为 `%LocalAppData%\LittleFocusDesktop`，而 `build.ps1` 只做了 exe 的分支命名。结果是 dev 版与正式版读写同一份 `data.xml` 和 `sync.xml`；在 dev 上测试 WebDAV 同步，会通过 `DataMerge` 把测试记录双向合并进正式数据。
- **改动**：
  - `scripts/build.ps1`：非 main 分支编译时注入 `/define:DEV`（与已有的分支命名同源判定，读 `.git/HEAD`，不依赖 git CLI）。
  - `src/Program.cs`：`#if UI_TEST` > `#elif DEV` > 默认正式目录；dev 走 `%LocalAppData%\LittleFocusDesktop-dev`，`sync.xml` 随目录一并隔离。
  - `src/MainForm.cs` / `src/MiniTimer.cs`：窗口标题、托盘提示、迷你计时条改用 `Program.AppName`，dev 版显示 `专注小站 (dev)`。
- **原因**：exe 分开但数据不分，等于「两个门牌、一个房间」——测试动作会直接落在真实数据上。
- **附带清理**：删除临时脚本 `_fix_garbled.py`。该脚本针对一个并不存在的「乱码文件名」：已扫描全仓库 6 个提交、所有分支、索引与磁盘，非 UTF-8 路径数为 0；git 显示的 `\345\210\206` 是 `core.quotepath` 的八进制转义，属显示转义而非存储乱码。
- **验证**：dev 构建 `FocusStation-v2.0.10-dev.exe` 内含 `LittleFocusDesktop-dev` 与 `专注小站 (dev)`；以 main 分支配置构建仍产出 `FocusStation-v2.0.10.exe`，数据目录保持正式路径，未受影响。
- **注意**：dev 版首次启动数据为空，不会自动复制正式数据；需要带真实数据复现时手动复制 `data.xml` / `sync.xml` 到 dev 目录。

---

## 2026-09-08 · 分支治理补齐（main/dev 双分支模型）

- **新增分支治理资产**（对应手册 2.2 / 2.3 / 2.4 / 3.4）：
  - `.github/workflows/ci.yml`：push dev 与 PR main 时自动 `build.ps1` → `test.ps1`；`concurrency` group 设 `cancel-in-progress: false`，`checkout` 用 `fetch-depth: 0` 保留完整历史；非生产环境自动注入 `PUSH_TEST=1`。
  - `.github/workflows/ops.yml`：`workflow_dispatch` 手动运维入口（`mode: build/test/build+test`），带参数校验，且守卫 `main` 禁止手动触发（生产守卫）。
  - `.github/PULL_REQUEST_TEMPLATE.md`：合并前检查清单，逐项对应手册（构建/测试/rebase/文档互锁/失效行为/回归用例/兼容旧数据）。
  - `docs/分支与发布流程.md`：分支模型、环境标记、CI/运维入口、合并流程、rebase 纪律、本地代理/凭据约定。
- **原因**：仓库已有 main+dev 分支，但缺"存在分支时应有的验证与运维入口"。本次按手册补齐，使 dev 的任何改动在合并前都有自动构建+测试把关，且生产分支有手动触发守卫。
- **未做（待实测）**：UI 测试在 CI 无显示器 runner 上的可行性待验证；若失败再据手册调整（如改为非阻塞或自托管 runner）。

---

## 2026-09-08 · 治理资产补齐 + 目录结构化（main 基线）

- **目录整理**：源码归入 `src/`，策略文档归入 `policies/`，脚本归入 `scripts/`，治理文档归入 `docs/`；同步更新 `build.ps1`/`test.ps1` 的资源与源码路径。`app.manifest` 保留在仓库根。
- **新增治理文档**（均不改动业务代码，低风险）：
  - `README.md`：项目概览、构建/运行、文档索引、分支约定（手册 3.2 文档与代码互锁）。
  - `docs/项目方案.md`：产品定位、架构、功能矩阵、成熟度、风险与路线图。
  - `docs/假设清单.md`：假设/验证/出错表现三列表 = 风险登记表（手册 0.2）。
  - `docs/工程实践对照.md`：对照手册逐条体检表。
  - `docs/CHANGELOG.md`：本文件。
- **`.gitignore`** 增补 `.workbuddy/`（本地记忆不入库）。
- **原因**：此前代码已满足手册 MVP 级要求，但缺"写下来的治理资产"。本次补齐文档层，使 main 成为可审计、可交接的稳定基线。
- **未做（明确排除）**：手册中调度双保险 / heartbeat / 多平台注册表对本机应用不适用，不补。

---

## 基线说明（v2.0.10）

以下为整理前已存在的核心能力，记于此以便追溯（非本次变更）：

- 凭据卫生：Windows 凭据加密，绝不入库，备份不含密钥（手册 1.1 ✅）。
- 状态版本：`AppData.Version == 1` + `AiDefaults.Migrate` 迁移（手册 1.2 ✅）。
- 幂等：RunId 去重、原子写、合并幂等，均有测试（手册 1.3 ✅）。
- fail-open：AI/同步失败不影响主计时（手册 1.4 ✅）。
- 测试隔离：`UI_TEST` 落 `testdata/`，不碰真实数据（手册 1.5 ✅）。
- 单调性闸门：合并取 `UpdatedUtc` 最新（手册 2.2 ✅）。
- 假成功检查：校验响应体结构（手册 2.3 ✅）。
- 状态可审计：`.bak`/`.damaged-*` 备份（手册 2.4 ✅ 部分）。
- **缺口（留给 dev 分支）**：本地结构化事件日志（手册 1.6）、事故→回归用例映射（手册 3.1）。
